...
TASK(Giro)
{
	...
	/* Comprobamos que distancia hay al girar a la derecha */    
	...
	// Obtenemos el valor de la distancia en la direccion derecha
	...
	// Obtener el valor de la distancia en la direccion izquierda
	...
	/* Si distancia derecha es mayor que la izquierda:
      ...
      En caso contrario:
        - El robot no se gira porque la direccion a tomar es la izquierda
   */
	...
	if(distanciaDcha > distanciaIzq){
		...
		//Si al final tiene que ir a la derecha almacenamos el valor
		movimientos[iterador] = 2;
		iterador++;
	} else{
		//En caso contrario almacenamos que gire a la izq
		movimientos[iterador] = 1;
		iterador++;
	}
...
TASK (Principal)
{
   ...
   /* La tarea Principal se ejecutara mientras la bandera de fin este desactivada (igual a 0) */
	...
	do{
	   ...
	   if(fin != 1){				
			...
			// Va hacia atras un poco para empezar de nuevo el reconocimiento
			nxt_motor_set_speed(NXT_PORT_B, -50, 1);    
			nxt_motor_set_speed(NXT_PORT_C, -50, 1);
			systick_wait_ms(600);
			
			movimientos[iterador] = (nxt_motor_get_count(NXT_PORT_B) + nxt_motor_get_count(NXT_PORT_C))/2;
			iterador++;
	...
	movimientos[iterador] = -1; //FIN
	iterador++;
	
	ecrobot_send_bt_packet(&iterador, sizeof(int));
	systick_wait_ms(100);
...
TASK (Pulsador){
	...
	// Obtenemos el valor del sensor de pulsacion para ver si ha sido activado
